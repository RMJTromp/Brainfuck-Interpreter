...
const codeBlockClassName = cn(
  ...
  '[&_.line]:w-full',
  '[&_.line]:relative',
  '[&_code]:font-["JetBrains_Mono",monospace]'
);

const brainfuckGrammer = {
  name: 'brainfuck',
  ...
  patterns: [
    {
      name: 'keyword.operator.pointer.brainfuck',
      match: '[<>]'
    },
    {
      name: 'keyword.operator.memory.brainfuck',
      match: '[+\\-]'
    },
    {
      name: 'keyword.operator.io.brainfuck',
      match: '[.,]'
    },
    {
      name: 'keyword.control.loop.brainfuck',
      match: '[\\[\\]]'
    },
    {
      name: 'comment.line.brainfuck',
      match: '[^<>+\\-.,\\[\\]]+'
    }
  ]
};

const getHighlighter = singleton(async () => {
  const highlighter = await createHighlighter({
    themes: ['github-light', 'github-dark-default'],
    langs: [],
  })

  const githubLight = highlighter.getTheme('github-light');
  const githubDark = highlighter.getTheme('github-dark-default');

  highlighter.setTheme({
    ...githubLight,
    settings: [
      ...(githubLight.settings || []),
      {
        scope: 'keyword.operator.pointer.brainfuck',
        settings: { foreground: '#CF222E' }
      },
      {
        scope: 'keyword.operator.memory.brainfuck',
        settings: { foreground: '#0550AE' }
      },
      {
        scope: 'keyword.operator.io.brainfuck',
        settings: { foreground: '#6639BA' }
      },
      {
        scope: 'keyword.control.loop.brainfuck',
        settings: { foreground: '#1F2328' }
      },
      {
        scope: 'comment.line.brainfuck',
